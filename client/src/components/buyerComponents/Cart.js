...
const CartPage = () => {
  ...
  const handleRemoveAll = async () => {
    ...
    const value = await axios
      .delete("http://localhost:7001/api/checkout/empty-cart", {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
  ...
  function ShoppingCart({
    ...
    // Ensure coupon code is sanitized before submission
    const handleApplyCouponSafe = () => {
      const sanitizedCode = DOMPurify.sanitize(textFieldValue);
      handleApplyCoupon(sanitizedCode);
    };
          ...
          <Card>
            <CardContent style={{ textAlign: "center" }}>
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                <Typography
                  variant="h5"
                  sx={{ mb: "20px", textAlign: "left", fontWeight: "900" }}
                >
                  Shopping Cart
                </Typography>
                <Button sx={{ textAlign: "right" }} onClick={handleRemoveAll}>
                  Remove All
                </Button>
              </Box>
              {cart ? (
                        ...
                        <TableRow key={item._id}>
                          <TableCell>
                            <img
                              ...
                              alt={DOMPurify.sanitize(item.product.title)} // Sanitize the alt text
                              style={{ width: "50px" }}
                            />
                          </TableCell>
                          <TableCell>
                            {DOMPurify.sanitize(item.product.title)}
                          </TableCell>
                          <TableCell>Rs.{item.product.price}.00</TableCell>
                          <TableCell>{item.count}</TableCell>
                          <TableCell>
                            Rs.{item.product.price * item.count}.00
                          </TableCell>
                          <TableCell>
                            <IconButton
            ...
            <CardContent>
              ...
              <Box sx={{ display: "flex", "& > :not(style)": { m: 1 } }}>
                ...
                <Button
                  ...
                  variant="contained"
                  sx={{ px: "60px" }}
                  onClick={handleApplyCouponSafe} // Use the safe handler
                >
                  Apply
                </Button>
              </Box>
            ...
            <CardContent>
              ...
              >
                ...
                <Typography sx={{ textAlign: "right" }}>
                  Rs.
                  {cart && cart.cartTotal && cart.tax
                    ? cart.cartTotal - cart.tax
                    : 0}
                </Typography>
              </Box>
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <Typography color="error" sx={{ textAlign: "right" }}>
                  {" "}
                  - Rs.
                  {cart && cart.cartTotal && cart.totalAfterDiscount
              ...
              <h3 align="center" fullWidth variant="contained" color="success">
              ...
              <Button
                ...
                to={`/payment/${cart}`} // Ensure cart is properly sanitized if used in URL
              >
                PayNow
              </Button>
            </CardContent>
